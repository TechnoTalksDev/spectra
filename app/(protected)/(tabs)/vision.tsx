...
export default function VisionPage() {
  ...
  const startRealtimeSession = async () => {
    try {
      ...
      // Keep actual mic track state in sync with UI state.
      ...
      const channel: any = pc.createDataChannel("oai-events", {
        ordered: true,
      });
      ...
      const endpoint =
        process.env.EXPO_PUBLIC_OPENAI_REALTIME_ENDPOINT ||
      ...
      if (isAzure) {
        ...
        const deploymentName = deploymentMatch
          ? deploymentMatch[1]
          : "gpt-realtime-mini";
        ...
        if (!tokenResponse.ok) {
          ...
          throw new Error(
            `Failed to get ephemeral token: ${tokenResponse.status} - ${errorText}`,
          );
        }
        ...
        console.log(
          "[Token] Ephemeral token received:",
          ephemeralToken ? `${ephemeralToken.substring(0, 10)}...` : "NONE",
        );
      } else {
        // Regular OpenAI
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to connect: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
  ...
  const stopRealtimeSession = () => {
    ...
    if (audioInputTokensRef.current > 0 || textInputTokensRef.current > 0) {
      console.log(
        "[Token Summary]   - Audio Input:",
        audioInputTokensRef.current,
      );
      console.log(
        "[Token Summary]   - Text Input:",
        textInputTokensRef.current,
        "(may include image tokens)",
      );
      if (cachedInputTokensRef.current > 0) {
        console.log(
          "[Token Summary]   - Cached:",
          cachedInputTokensRef.current,
        );
      }
    }
    console.log(
      "[Token Summary]   - Images Sent:",
      imagesSentRef.current,
      "(billed separately)",
    );
    console.log("[Token Summary] OUTPUT TOKENS:");
    console.log(
      "[Token Summary]   Total Audio Output:",
      totalOutputTokensRef.current,
    );

    // Calculate costs (per 1M tokens)
    const audioInputCost = (audioInputTokensRef.current / 1_000_000) * 10;
    const textInputCost = (textInputTokensRef.current / 1_000_000) * 0.6;
    const cachedInputCost = (cachedInputTokensRef.current / 1_000_000) * 0.3;
    ...
    console.log(
      "[Token Summary]   Estimated Input Cost: $" + totalInputCost.toFixed(4),
    );
    console.log(
      "[Token Summary]   Estimated Output Cost: $" + totalOutputCost.toFixed(4),
    );
    console.log(
      "[Token Summary]   Total Estimated Cost: $" + totalCost.toFixed(4),
    );
    console.log(
      "[Token Summary] ═══════════════════════════════════════════════════",
    );
  ...
  const handleToggleMute = () => {
    if (!isSessionActive || !localStreamRef.current) return;
  ...
  const sendImageOverDataChannel = async (base64Image: string) => {
    ...
    try {
      ...
      console.log(
        "[Auto Snapshot] Sending Image event to GPT Realtime (size: " +
      ...
      console.log(
        "[Auto-Snapshot] Image event successfully (Total images sent:",
        imagesSentRef.current,
        ")",
      );
      ...
      Alert.alert(
        "Image Send Failed",
        err instanceof Error ? err.message : "Could not send image to AI",
      );
    }
  };
  ...
  if (!permission.granted) {
    return (
      <View
        style={[
          styles.container,
          { backgroundColor: colors.background.main, paddingTop: insets.top },
        ]}
      >
      ...
      >
        <View style={styles.permissionContainer}>
          ...
          <Text
            style={[styles.permissionText, { color: colors.text.secondary }]}
          >
            We need access to your microphone to provide voice interaction and
            assistance.
          </Text>
          <GlassButton
            title="Grant Permission"
